...
\begin{defn}
    (Convergence): $\left\{ x_n \right\}_{n = 1}^{\infty} \subseteq X $ converges to $x$ in $(X, p)$ if 
...
\begin{defn}
    (Uniform Continuity): $f : (X,p) \to (\R, \left| . \right| )$ uniformly continuous if $f$ has a 
...
\end{rem}
\begin{defn}
    (Completeness): We say $(X, p)$ \textit{complete} if every Cauchy sequence in $(X, p)$ converges
    to a point in $X$. 
\end{defn}
...
\begin{defn}
    (Open Cover, Compactness): $\left\{ X_\lambda \right\}_{\lambda \in \Lambda} \subseteq 2^X$
    \marginpar{$2^X$ denotes the power set of $X$, i.e. the set of all subsets of $X$.},
...
\begin{defn}
    (Totally Bounded, $\epsilon$-nets): $(X, p)$ is \textit{totally bounded} if 
...
\begin{defn}
    (Sequentially Compact): $(X, p)$ \textit{sequentially compact} if every sequence in $X$ has
    a convergent subsequence whose limit is in $X$. 
\end{defn}

\begin{defn}
    (Relatively/Pre-Compact): $E \subseteq X$ \textit{precompact} if $\overline{E}$ compact.   
\end{defn}
...
\begin{defn}
    (Density/Separability): A set $D \subseteq X$ is called \textit{dense} in $(X, p)$ if for every
    \marginpar{If $A$ dense in $X$, then $\overline{A}$ dense in $X$}  
...
\begin{defn}
    (Equicontinuous): A family $\mathcal{F} \subseteq C(X)$ is called \textit{equicontinuous} at
...
\begin{defn}
    (Pointwise, Uniform Boundedness): $\left\{ f_n \right\}$ pointwise bounded if $\forall x \in X, 
    \exists M(x) > 0$ such that $\left| f_n(x) \right| \leq M(x) \ \forall n$, and uniformly bounded
    if such an M exists independent of $X$.    
\end{defn}

\begin{defn}
    (Uniform Equicontinuous): $\mathcal{F} \subseteq C(X)$ is uniformly equicontinuous on
...
\begin{lem}
    (Arzelà-Ascoli Lemma): Let $X$ be separable and let $\{f_{n}\}_{n=1}^{\infty} \subseteq C(X)$
...
\begin{thm}
    (Arzelà-Ascoli Theorem): Let $X$ be compact and let $\{f_{n}\}_{n=1}^{\infty} \subseteq C(X)$
...
\begin{defn}
    (Hollow/Nowhere Dense): We say a set $E$ is \textit{hollow} if $\interior(E) = \emptyset$.
...
\begin{thm}
    Let $(X, p)$ be complete. Let $\{f_{n}\}_{n=1}^{\infty} \subseteq C(X)$ s.t. $f_n \to f$ pointwise in $X$. Then there is a dense subset $D \subseteq X$ where $\{f_{n}\}_{n=1}^{\infty}$ is pointwise equicontinuous on $D$ and $\forall x_0 \in D$, $f$ is continuous at $x_0$.
\end{thm}
