...
\noindent{\small\sffamily\color{gray} McGill University \,|\, Winter 2026}

\vspace{0.5cm}

\noindent{\Huge\bfseries\sffamily\color{MidnightBlue} MATH 455}\\[0.3cm]
\noindent{\LARGE\sffamily Lecture Notes}

\vspace{0.8cm}

\noindent{\large Charles Zitella}\\[0.2cm]
\noindent{\small\color{gray} \today}

\vspace{0.5cm}

\noindent{\small\itshape Based on lectures by Prof.\ Jessica Lin}
...
\begin{exmp}
    ...
    \begin{minipage}[t]{0.4\textwidth}
        \vspace{0pt}
        \centering
        \scalebox{0.8}{\input{fn_graph.tex}}
        {\footnotesize{\captionof{figure}{The sequence $f_n(x)=x^n$ is not equicontinuous.}}}
    \end{minipage}
\end{exmp}
\begin{comment}
\begin{marginfigure}
...
\begin{defn}[limit point, closure, closed set]
    If $E \subseteq X, x \in X$ is a limit point if $\forall $ neighbourhood $U_x$ of $x$,
    \[
        U_x \cap E \neq \emptyset.
    \]
    We say $\overline{E} = \left\{  \text{All limit points of E } \right\} $, is the closure of $E$.
    
    \noindent We say $E$ is closed if $E = \overline{E}$.   
\end{defn}
...
\begin{defn} [Density, Separability]
    $D \subseteq X$ is dense if $\forall $ non-empty open set $U$, $U \cap D \neq \emptyset$.
    $\iff \overline{D} = X$.
    
    $(X, \mathcal{T})$ is separable iff $X$ contains a countable dense set.  
...
We need topologies with more structure.
...
\begin{exmp}
    ...
    $B = \left\{ (a, b) : a < b \right\} \cup \left\{ (a, b)\setminus K : a < b \right\}$.
    We check 
    \begin{itemize}
...
\begin{proof}
    \begin{align*}
        ...
        & \iff \forall y \in \left\{ x \right\}^c, \exists \text{ neighbourhood } U_y \subseteq \left\{ x \right\}^c \\
        & \iff x \notin U_y
    \end{align*}
\end{proof}

\begin{rem}
    so $(X, \mathcal{T})$ normal $\Rightarrow (X, \mathcal{T})$ regular.  
\end{rem}
