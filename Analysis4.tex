...
\begin{defn}
    ...
    \end{itemize}
    We say $(X, \mathcal{T})$ is a \textit{topological space}.  
    \newline If $E \in \mathcal{T}$, then we call $E$ an open set (with respect to $\mathcal{T}$).  
    \newline If $x \in X$, a set $E \in \mathcal{T}$ containing x is called a \textit{neighborhood} of $x$.

\end{defn}

\begin{rem}
    By definition of $\mathcal{T}$, $E \in \mathcal{T}$ iff $\forall x \in E, \exists$ a neighbourhood of $x$,
    contained in $E$. (consistent with metric space definition of open set)     
\end{rem} 
...
\begin{exmp}[Discrete Topology]
    ...
    \Rightarrow E$ is open. Then, $\mathcal{T} = P(X) = \left\{ \text{All possible subsets of } X \right\}
     $ is a topology on $X$, called the discrete topology, 
    and it contains all subsets of $X$.
...
\begin{exmp}[Relative Topology]
    ...
    So $\forall y \in \N, \forall x \in \R, r > 0, $ \[B(x, r) \cap \N = \begin{cases}
        ...
        \emptyset, & y \notin B(x, r)
    \end{cases}\]
    Thus, $\mathcal{T}_{\N} = P(\N)$, the discrete topology on $\N$.
    
    If $X = \R, Y = [0, 1)$, then $\mathcal{T}_{[0, 1)} = \left\{ U \cap [0, 1) : U \subseteq \R 
    \text{ open}\right\}$
    So the set $[0, \frac{1}{2}) = [0, 1) \cap (-1, \frac{1}{2}) \in \mathcal{T}_{[0, 1)}$.
    So $[0, \frac{1}{2})$ is relatively open in $Y = [0, 1)$ (belings to the relative topology on $Y$).
\end{exmp}
...
\begin{defn}[base/neighbourhood base]
    ...
    neighborhoods of $x$. We call $\mathcal{B}_x$ a neighbourhood base at $x$ if $\forall$ neighborhood 
    of $x$ (call it $u$), $\exists B \in \mathcal{B}_x$ such that $B \subseteq u$.  
    We say $\mathcal{B}$, a collection of open sets, is a base for $\mathcal{T}$ if $\forall x \in X$,
    $\exists$ a neighbourhood base $\mathcal{B}_x \subseteq \mathcal{B}$ at $x$.    
...
\begin{defn}[first countable/second countable]
    ...
    \begin{itemize}
        \item We say $(X, \mathcal{T})$ is \textit{first countable} if there is a countable neigbourhood
        base at each $x \in X$;
        \item We say $(X, \mathcal{T})$ is \textit{second countable} if there is a countable base $B$
        of $\mathcal{T}$. 
    \end{itemize}
\end{defn}
...
\begin{proof}
    ...
    and
    \[
        u = \bigcup_{x \in u} \left\{ x \right\} \subseteq \bigcup_{x \in u} B_x \Rightarrow u = \bigcup_{x \in u} B_x.
    \] 
...
\begin{prop}
    Let $X \neq \emptyset$. Let $\mathcal{B} \subseteq P(X)$ be a collection of sets. Then $\mathcal{B}$
    is a base for some topology $\mathcal{T}$ iff 
    \begin{enumerate}
...
\begin{proof}
    ...
    \cap U_2$, by definition of $\mathcal{T}$, $\exists B_1 \in \mathcal{B}$ s.t. $x \in B_1 \subseteq U_1$,
    and $\exists B_2 \in \mathcal{B}$ s.t. $x \in B_2 \subseteq U_2$.
...
Observe that the properties which define $\mathcal{T}$ are closed under intersections. Let 
$\psi \subseteq P(X)$ be a collection of sets. Then
\[
    \mathcal{T}(\psi) = \bigcap \left\{ \text{All topologies containing } \psi \right\} = \text{
        topology generated by } \psi
\]   
...
\begin{defn}[weaker/coarser vs. stronger/finer]
    ...
    \mathcal{T}_2$, we say $\mathcal{T}_1$ is a weaker/coarser topology than $\mathcal{T}_2$ (fewer
    open sets), and $\mathcal{T}_2$ is a stronger/finer topology than $\mathcal{T}_1$ (more open sets).   
\end{defn}

\begin{exmp}
    Trivial topology is the weakest topology on $X$ and discrete topology is the strongest topology 
    on $X$. So $\mathcal{T}(\psi)$ is the weakest topology containing $\psi$.
\end{exmp}

\begin{prop}
    Let $\psi \subseteq P(X)$. Then 
    \[
        \mathcal{T}(\psi) = \left\{ \emptyset, X, \bigcup \left\{ \text{
            finite intersections of sets in } \psi \right\}  \right\} 
    \]  
\end{prop}
\begin{proof}
    \textbf{Claim:} $\mathcal{B} = \left\{ \emptyset, X, \text{ finite intersections of elements of } 
    \psi \right\} $ form a base.
    $\mathcal{B}$ satisfies (1) and (2) of the previous proposition, so $\mathcal{B}$ is a base for 
    some topology 

    \[
        \mathcal{T} = \left\{ \emptyset, X, \bigcup \left\{ \text{finite intersections of elements of } 
        \psi \right\}  \right\} 
    \] 
    Observe that $\mathcal{T} \subseteq \left\{ \text{any topology which contains } \psi \right\} $ 
    $\Rightarrow \mathcal{T} \subseteq \mathcal{T}(\psi)$. $\mathcal{T}$ tilda is also a topology,
    which contains $\psi$. So $\mathcal{T}(\psi) \subseteq \mathcal{T}$. Thus, $\mathcal{T} = 
    \mathcal{T}(\psi)$.   
\end{proof}
